...
public class PanelLogin extends JFrame {
	...
	private Usuario usu ;
    private List<Workouts> workoutsDisp;
		    ...
		    public void actionPerformed(ActionEvent e) {
		        ...
		        try {
		            ...
		            if (!resultados.isEmpty()) {
		                DocumentSnapshot usu = resultados.get(0);

		                int nivelActual = usu.contains("nivelActual") ? usu.getLong("nivelActual").intValue() : 0;

		                // Cargar workouts
		                List<Workouts> workouts = new ArrayList<>();
		                List<QueryDocumentSnapshot> dcWorkouts = db.collection("workouts").get().get().getDocuments();

		                for (QueryDocumentSnapshot doc : dcWorkouts) {
		                    workouts.add(new Workouts(
		                        doc.getId(),
		                        doc.getLong("numEjers").intValue(),
		                ...
		                PanelWorkouts siguiente = new PanelWorkouts(usu, workoutsDisp);
		                siguiente.setVisible(true);
		                dispose();

		            } else {
		                JOptionPane.showMessageDialog(null, "Usuario o contraseña incorrectos o no existe en la base de datos.");
		            }
